    ...
    client.on("message", async (message) => {
        ...
        function anti_double_messages() {
            ...
            if (oldmessage.toLowerCase() === message.content.toLowerCase().replace(/\s+/g, '')) {
                return console.log("DOUPLIZIERTE NACHRICHT, keine Rankingpunkte sorry!");
            }
        ...
        function LEVELUP() {
                ...
                const embed = new Discord.MessageEmbed()
                    .setAuthor(`Hallo ${message.author.tag}, Das ist dein aktueller Rank`, message.member.user.displayAvatarURL({
                        dynamic: true
                    }))
                    .setDescription(`Du bist ein Level aufgestiegen: **\`${newLevel}\`**! (Punkte: \`${newPoints}\` / \`${newneededPoints}\`) `)
                    .setColor(embedcolor);
        ...
        function rank(the_rankuser) {
            ...
            try {
                ...
                if (!rankuser) return message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
                ...
                for (const data of top10) {
                    try {
                        ...
                        i = `Fehler beim Zählen des Rangs`;
                        break;
                    }
                }
                //math
                ...
                if (client.points.get(key, `level`) === undefined) i = `Kein Rang`;
                ...
                rank.build()
                    .then(data => {
                        ...
                        //define embed
                        const embed = new Discord.MessageEmbed()
                            .setTitle(`Hallo ${rankuser.username}, Das ist dein aktueller Rank`)
                            .setColor(embedcolor)
            ...
            if(isNaN(maxnum)) {
                console.log("maximum_leaderboard KEINE ZAHL")
                maxnum = 50;}
            ...
            for (let i = 10; i <= maxnum; i += 10) {
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                    .setDescription(`Top ${i<orilent?i:orilent}/${orilent} Rangordnung:`)
                    .setColor(embedcolor);
                for (const data of top) {
                    j++;
                    try {
                        embed.addField(`**${j}**. \`${data.usertag}\``, `**Punkte:** \`${data.points.toFixed(2)}\` / \`${data.neededpoints}\` | **Level:** \`${data.level}\``);
                    } catch {
                        embed.addField(`**${j}**. \`${data.usertag}\``, `**Punkte:** \`${data.points.toFixed(2)}\` / \`${data.neededpoints}\` | **Level:** \`${data.level}\``);
                    }
                }
        ...
        async function leaderboard() {
            ...
            const lbembed = await message.channel.send(
                `**Aktuelle Seite - ${currentPage + 1}/${embeds.length}**`,
                embeds[currentPage]
            );
                ...
                try {
                    if (reaction.emoji.name === "⏩") {
                        if (currentPage < embeds.length - 1) {
                            currentPage++;
                            lbembed.edit(`**Aktuelle Seite - ${currentPage + 1}/${embeds.length}**`, embeds[currentPage]);
                        }
        ...
        function setxpcounter(){
            ...
            /**
                 ...
                 * @info you can tag him
                 */
                if (!args[0]) return message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
                ...
                const key = `${message.guild.id}-${rankuser.id}`;
                databasing(rankuser);
                if (!args[1]) return message.reply("BITTE FÜGEN SIE PUNKTE HINZU! Verwendung: `setxpcounter @USER 2`");
                client.points.set(key, Number(args[1]), `xpcounter`); //set points to 0
                const embed = new Discord.MessageEmbed()
                .setColor(embedcolor)
                .setDescription(`XP COUNTER erfolgreich auf \`${args[1]}x\` für: \`${rankuser.tag}\``)
                message.reply(embed);
            } catch (error) {
                console.log(error.stack)
                message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
            }
        }

        function setglobalxpcounter(){
            try {
                if (!args[0]) return message.reply("BITTE FÜGEN SIE PUNKTE HINZU! Verwendung: `setglobalxpcounter 2`");
                client.points.set(message.guild.id, Number(args[0]), `setglobalxpcounter`); //set points to 0
                const embed = new Discord.MessageEmbed()
                .setColor(embedcolor)
                .setDescription(`Global XP COUNTER wurde erfolgreich auf \`${args[0]}x\` für: \`${message.guild.name}\``)
                message.reply(embed);
            } catch {
            }
        }
        ...
        function addpoints(amount) {
            try {
                ...
                const neededPoints = client.points.get(key, `neededpoints`);
                let leftpoints = neededPoints - curPoints;
                if (!args[1] && !amount) return message.reply("BITTE FÜGEN SIE PUNKTE HINZU! Verwendung: `Addpoints @USER 100`");
                ...
                function addingpoints(toaddpoints, leftpoints) {
                    if (toaddpoints >= leftpoints) {
                        ...
                        //THE INFORMATION EMBED
                        const embed = new Discord.MessageEmbed()
                            .setAuthor(`Hallo ${rankuser.tag}, Das ist dein aktueller Rank`, rankuser.displayAvatarURL({
                                dynamic: true
                            }))
                            .setDescription(`Du bist mit dein Level aufgestiegen: **\`${newLevel}\`**! (Punkte: \`${newPoints + toaddpoints - leftpoints}\` / \`${newneededPoints}\`) `)
                            .setColor(embedcolor);
                ...
                const embed = new Discord.MessageEmbed()
                    .setColor(embedcolor)
                    .setDescription(`Erfolgreich hinzugefügt \`${toaddpoints} Punkte\` an: \`${rankuser.tag}\``)
                message.reply(embed);
        ...
        function setpoints() {
            try {
                ...
                if (!args[1]) return message.reply("BITTE PUNKTE ZUM SET HINZUFÜGEN! Verwendung: `Addpoints @USER 100`");
                ...
                function addingpoints(toaddpoints, neededPoints) {
                    if (toaddpoints >= neededPoints) {
                        ...
                        const embed = new Discord.MessageEmbed()
                            ...
                            .setDescription(`Du bist mit dein Level aufgestiegen **\`${newLevel}\`**! (Punkte: \`${newPoints}\` / \`${newneededPoints}\`) `)
                            .setColor(embedcolor);
                        //send ping and embed message
                        message.channel.send(rankuser, embed);
                        ...
                        client.points.set(key, Number(toaddpoints), `Punkte`)
                    }
                }

                const embed = new Discord.MessageEmbed()
                    .setColor(embedcolor)
                    .setDescription(`Erfolgreich gesetzt \`${toaddpoints} Punkte\` an: \`${rankuser.tag}\``)
                message.channel.send(embed);
                rank(rankuser); //also sending the rankcard
            } catch (error) {
                console.log(error.stack)
        ...
        function removepoints(amount) {
            try {
                ...
                if (!args[1] && !amount) return message.reply("BITTE FÜGEN SIE PUNKTE ZUM ENTFERNEN HINZU! Verwendung: `Addpoints @USER 100`");
                if (!amount) amount = Number(args[1]);
                ...
                function removingpoints(amount, curPoints) {
                    if (amount > curPoints) {
                        ...
                        if (client.points.get(key, `level`) == 1) return message.reply("BEREITS BEI 0 PUNKTEN");
                        ...
                        const embed = new Discord.MessageEmbed()
                            ...
                            .setDescription(`Du bist mit dein Level aufgestiegen **\`${newLevel}\`**! (Punkte: \`${newPoints - amount + removedpoints}\` / \`${newneededPoints}\`) `)
                            .setColor(embedcolor);
                ...
                const embed = new Discord.MessageEmbed()
                    .setColor(embedcolor)
                    .setDescription(`Erfolgreich entfernt \`${amount} Points\` from: \`${rankuser.tag}\``)
                message.reply(embed);
                rank(rankuser); //also sending the rankcard
            } catch (error) {
                console.log(error.stack)
        ...
        function addlevel() {
            try {
                ...
                if (!args[1]) return message.reply("Bitte fügen Sie die Anzahl der Level hinzu, zu denen Sie hinzufügen möchten! Verwendung: addlevel @User 4");
                if (Number(args[1]) < 0) args[1] = 0;
                ...
                .setColor(embedcolor)
                .setDescription(`Erfolgreich hinzugefügt ${args[1]} Stufen bis: \`${rankuser.tag}\``)
        ...
        function setlevel() {
            try {
                ...
                if (!args[1]) return message.reply("Bitte fügen Sie die Anzahl der Level hinzu, auf die Sie einstellen möchten! Verwendung: Setlevel @User 3");
                ...
                const sssembed = new Discord.MessageEmbed()
                .setColor(embedcolor)
                .setDescription(`Erfolgreich gesetzt \`${rankuser.tag}\` zum Level: ${args[1]}`)
                message.reply(sssembed);
            } catch (error) {
                console.log(error.stack)
                message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
        ...
        function removelevel() {
            try {
                ...
                databasing(rankuser);
                let newLevel = client.points.get(key, `level`);
                if (!args[1]) return message.reply("Bitte fügen Sie die Anzahl der Level hinzu, die Sie entfernen möchten! Verwendung: removelevel @User 4");
                if (Number(args[1]) < 0) args[1] = 0;
                for (let i = 0; i < Number(args[1]); i++) {
                    client.points.set(key, 0, `points`); //set points to 0
                ...
                const sssembed = new Discord.MessageEmbed()
                .setColor(embedcolor)
                .setDescription(`Erfolgreich entfernt \`${args[0]}\` Stufen ab:  \`${rankuser.tag}\``)
                message.reply(sssembed);
            } catch (error) {
                console.log(error.stack)
                message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
        ...
        function resetranking() {
            try {
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`Du bist mit dein Level aufgestiegen **\`1\`**! (Punkte: \`0\` / \`400\`) `)
                    .setColor(embedcolor);
                message.channel.send(rankuser, embed);
                rank(rankuser); //also sending the rankcard
                const sssembed = new Discord.MessageEmbed()
                .setColor(embedcolor)
                .setDescription(`Erfolgreich zurückgesetztes Ranking von:  \`${rankuser.tag}\``)
                message.reply(sssembed);
            } catch (error) {
                console.log(error.stack)
                message.reply("BITTE FÜGEN SIE EINEN RANKUSER HINZU!");
        ...
        function registerall() {
            ...
            const embed = new Discord.MessageEmbed()
            .setColor(embedcolor)
            .setDescription(`Alle erfolgreich registriert`)
            message.reply(embed);
        }
        ...
        function resetrankingall() {
            ...
            const embed = new Discord.MessageEmbed()
            .setColor(embedcolor)
            .setDescription(`Alle erfolgreich zurückgesetzt`)
            message.reply(embed);
        }
        ...
        function addrandomall() {
            ...
            const embed = new Discord.MessageEmbed()
            .setColor(embedcolor)
            .setDescription(`Erfolgreich hinzugefügt ${args[0]} Punkte für alle`)
            message.reply(embed);
        }
        ...
        function levelinghelp() {
            const embed = new Discord.MessageEmbed()
                ...
                .addFields([{
                    ...
                    {
                        name: "`leaderboard`",
                        value: ">>> *Zeigt die Top 10 Bestenliste*",
                        inline: true
                    },
                    {
                        name: "`setxpcounter <@USER> <AMOUNT>`",
                        value: ">>> *Ändert die Menge, wie viel gezählt werden soll, x1, x2, x3, ...*",
                        inline: true
                    },

                    {
                        name: "`addpoints <@User> <Amount`",
                        value: ">>> *Hinzufügen einer bestimmten Anzahl von Punkten zu einem Benutzer*",
                        inline: true
                    },
                    {
                        name: "`setpoints <@User> <Amount`",
                        value: ">>> *Festlegen einer bestimmten Anzahl von Punkten für einen Benutzer*",
                        inline: true
                    },
                    {
                        name: "`removepoints <@User> <Amount`",
                        value: ">>> *Entfernen einer bestimmten Anzahl von Punkten für einen Benutzer*",
                        inline: true
                    },

                    {
                        name: "`addlevel <@User> <Amount`",
                        value: ">>> *Hinzufügen einer bestimmten Anzahl von Ebenen zu einem Benutzer*",
                        inline: true
                    },
                    {
                        name: "`setlevel <@User> <Amount`",
                        value: ">>> *Festlegen einer bestimmten Anzahl von Ebenen für einen Benutzer*",
                        inline: true
                    },
                    {
                        name: "`removelevel <@User> <Amount`",
                        value: ">>> *Entfernen einer bestimmten Anzahl von Ebenen für einen Benutzer*",
                        inline: true
                    },

                    {
                        name: "`resetranking <@User>`",
                        value: ">>> *Setzt das Ranking eines Benutzers zurück*",
                        inline: true
                    },
                    {
                        name: "`setglobalxpcounter <AMOUNT>`",
                        value: ">>> *Legt den globalen XP-Zähler für diese Gilde fest, Standard 1*",
                        inline: true
                    },
                    {
                        name: "\u200b",
                    ...
                    {
                        name: "`registerall`",
                        value: ">>> *Registrieren aller Personen auf dem Server in der Datenbank*",
                        inline: true
                    },
                    {
                        name: "`resetrankingall`",
                        value: ">>> *Zurücksetzen der Rangfolge aller Benutzer auf diesem Server*",
                        inline: true
                    },
                    {
                        name: "`addrandomall`",
                        value: ">>> *Fügen Sie allen eine zufällige Anzahl von Punkten hinzu*",
                        inline: true
                    }
                ])
            message.channel.send(embed)
